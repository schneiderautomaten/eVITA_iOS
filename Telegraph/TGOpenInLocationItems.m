...
- (void)performOpenIn
{
    ...
    if (directions)
    {
        openInURL = [NSURL URLWithString:[[NSString alloc] initWithFormat:@"comgooglemaps-x-callback://?daddr=%@&directionsmode=driving&x-success=eVITA://?resume=true&&x-source=eVITA", coordinatePair]];
    }
    else
    {
        openInURL = [NSURL URLWithString:[[NSString alloc] initWithFormat:@"comgooglemaps-x-callback://?center=%@&q=%@&x-success=eVITA://?resume=true&&x-source=eVITA", coordinatePair, coordinatePair]];
    }
