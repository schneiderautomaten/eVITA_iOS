#import "TGGoogleDriveThumbnailDataSource.h"
...
@implementation TGGoogleDriveThumbnailDataSource
...
+ (NSString *)uriPrefix
{
    return @"google-drive-thumbnail://";
}

- (bool)canHandleUri:(NSString *)uri
{
    return [uri hasPrefix:[TGGoogleDriveThumbnailDataSource uriPrefix]];
}
...
{
    NSDictionary *args = [TGStringUtils argumentDictionaryInUrlString:[uri substringFromIndex:[[NSString alloc] initWithFormat:@"%@?", [TGGoogleDriveThumbnailDataSource uriPrefix]].length]];
...
{
    if ([TGGoogleDriveThumbnailDataSource imageAddressForUri:uri size:NULL] == nil)
    {
        if (completion)
            completion([TGGoogleDriveThumbnailDataSource resultForUnavailableImage]);
        return nil;
    }
    ...
    [taskManagementQueue() dispatchOnQueue:^
     {
         ...
         {
             TGDataResource *result = [TGGoogleDriveThumbnailDataSource _performLoad:uri isCancelled:isCancelled];
             ...
             if (completion != nil)
                 completion(result != nil ? result : [TGGoogleDriveThumbnailDataSource resultForUnavailableImage]);
         }];
         
         if ([TGGoogleDriveThumbnailDataSource _isDataLocallyAvailableForUri:uri])
         {
             [previewTask executeWithWorkerTask:workerTask workerPool:workerPool()];
         }
         else
         {
             NSString *imageUrl = [TGGoogleDriveThumbnailDataSource imageAddressForUri:uri size:NULL];
...
- (TGDataResource *)loadDataSyncWithUri:(NSString *)uri canWait:(bool)canWait acceptPartialData:(bool)__unused acceptPartialData asyncTaskId:(__autoreleasing id *)__unused asyncTaskId progress:(void (^)(float))__unused progress partialCompletion:(void (^)(TGDataResource *))__unused partialCompletion completion:(void (^)(TGDataResource *))__unused completion
{
    ...
    if ([TGGoogleDriveThumbnailDataSource imageAddressForUri:uri size:NULL] == nil)
        return [TGGoogleDriveThumbnailDataSource resultForUnavailableImage];
    ...
    return [TGGoogleDriveThumbnailDataSource _performLoad:uri isCancelled:nil];
}
...
+ (TGDataResource *)_performLoad:(NSString *)uri isCancelled:(bool (^)())isCancelled
{
    ...
    NSString *imageUrl = [TGGoogleDriveThumbnailDataSource imageAddressForUri:uri size:&size];
