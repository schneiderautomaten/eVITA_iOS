...
@property (nonatomic) bool messageSentToeVITA;
...
- (id)initWithShowKeyboard:(bool)__unused showKeyboard phoneNumber:(NSString *)phoneNumber phoneCodeHash:(NSString *)phoneCodeHash phoneTimeout:(NSTimeInterval)phoneTimeout messageSentToeVITA:(bool)messageSentToeVITA messageSentViaPhone:(bool)messageSentViaPhone
{
    ...
    if (self)
    {
        ...
        _messageSentToeVITA = messageSentToeVITA;
...
- (void)loadView
{
    ...
    [self makeLabelWithFormattedText:_noticeLabel text:_messageSentToeVITA ? TGLocalized(@"Login.CodeSentInternal") : (_messageSentViaPhone ? TGLocalized(@"Login.CodeSentCall") : TGLocalized(@"Login.CodeSentSms"))];
    ...
    _timeoutLabel.hidden = _messageSentToeVITA || _phoneTimeout >= (3600.0 - DBL_EPSILON);
    ...
    _didNotReceiveCodeButton.hidden = !_messageSentToeVITA;
...
{
    if (_countdownTimer == nil && !_alreadyCountedDown && !_messageSentToeVITA)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^
        {
            ...
            else
            {
                ...
                if (resultCode == TGSignInResultNotRegistered)
                {
                    ...
                    [TGAppDelegateInstance saveLoginStateWithDate:stateDate phoneNumber:_phoneNumber phoneCode:_phoneCode phoneCodeHash:_phoneCodeHash codeSentToeVITA:false codeSentViaPhone:false firstName:nil lastName:nil photo:nil resetAccountState:nil];
    ...
    else if ([path hasPrefix:@"/tg/service/auth/sendCode/"])
    {
        dispatch_async(dispatch_get_main_queue(), ^
        {
            ...
            if (_messageSentToeVITA)
            {
                ...
                {
                    int stateDate = [[TGAppDelegateInstance loadLoginState][@"date"] intValue];
                    [TGAppDelegateInstance saveLoginStateWithDate:stateDate phoneNumber:_phoneNumber phoneCode:nil phoneCodeHash:_phoneCodeHash codeSentToeVITA:false codeSentViaPhone:false firstName:nil lastName:nil photo:nil resetAccountState:nil];
                    
                    bool messageSentViaPhone = [(((SGraphObjectNode *)result).object)[@"messageSentViaPhone"] intValue];
                    
                    TGLoginCodeController *controller = [[TGLoginCodeController alloc] initWithShowKeyboard:(_codeField.isFirstResponder) phoneNumber:_phoneNumber phoneCodeHash:_phoneCodeHash phoneTimeout:_phoneTimeout messageSentToeVITA:false messageSentViaPhone:messageSentViaPhone];
